...
## C-20: Season records through the weekly loop, and strength tiers (Checkpoint A review, 2026-09-25)
- Change: calibration gains weekly-loop seasons, which play each regular season through the weekly advance and judge the season records (D-24); a full run plays 100 of them. `league.teamSpread` (an independent uniform offset, sd 0.2) gives way to strength tiers: `league.tiers` [10, 12, 10], `tierGap` 0.33, `tierWidth` 0.2. `sim.protectPassCut` 0.4 to 0.5. The weekly loop's AI fixes (D-24) apply to both modes.
- Effect: on seed 1 the weekly loop's win sd was 2.87 in 24 seasons against 2.98 for replays. A wider single spread raised it only with more extremes (a 0.25 ladder: 2.98, and 10 two-win teams per decade), and tiers added on top of the uniform offset pushed perfect or winless teams to 8 to 20 per 100 seasons. With tiers alone the loop reached 3.05, but replay blowouts rose to 26.2% and the passing and receptions leaders passed their bands; firmer lead protection brought them back. Final 100-season runs (100 replays and 100 weekly-loop seasons): seed 1 passes 62 of 62 targets (loop win sd 3.06, perfect or winless 4.0 per 100, two-win teams 5.8 per decade, 15-win teams 5.4; blowouts 25.8%, favorites 66.6%, passing leader 5,123, receptions leader 136) and seed 2 passes 62 (win sd 3.03, perfect or winless 5.0, blowouts 25.9%). A full run takes about 12 minutes on 4 workers.

## C-21: Aging through the offseason (M10 close, 2026-09-25)
- Change: calibration gains leagues chained through the offseason (3 of 10 seasons in a full run) and 16 aging metrics (D-32). Progression's curves count from their own peak ages (QB 30 to 27, TE and LB 27 to 26); `potentialRoom` 8 to 12 and `potentialBounds` [0.25, 1.5] to [0, 1.5], so growth stops at potential; usual retirement ages half a year sooner for every group but QB and specialists; `resign.maxAge` 32 to 30; the cutdown's `cutDraftBonus` [16, 14, 12, 10, 9, 8, 7] and `cutUndraftedBonus` 6; the stand-in draft classes' `classQualityByGroup` (QB -0.8, TE -0.3, RB +0.6, OL and DB +0.2).
- Effect: before, a 10-season chain raised the quarterbacks' week 1 average from 72.8 to 83.1 and the league's by 1.9, kept 6.3 rookies and 10.0 players 30 and older a team, and put the quarterback peak at 32.2. The seed 1 100-season run passes all 76 targets: average age 26.34, experience 4.72 counting the current season, 9.3 rookies and 7.9 players 30 and older a team, league drift −0.2 and the largest group's 2.1 over 10 seasons, peak ages QB 29.7, RB 25.3, WR 26.5, TE 27.5, OL 28.1, DL 28.1, LB 27.6, DB 26.6; the weekly loop's season records still pass (win sd 3.06). A full run takes about 22 minutes on 4 workers.

## C-22: A softer parity pull, tier sizes by league, and the NFL's perfect or winless rate (M10, 2026-09-26)
- Change: `seasons.perfectOrWinlessPer100` is sourced to the NFL's 3 perfect or winless teams in the 44 full seasons from 1981 to 2025, about 6.8 per 100: pass 3 to 11, warn 1 to 14 (was 0 to 5 and 0 to 8, from spec 23.2's "a handful at most"). `league.balanceKeep` 0.2 to 0.4, so a roster keeps twice the excess past `balanceFrom`; `league.tiers` [10, 12, 10] gives way to `tierSizes`, 8 to 12 contenders and 8 to 12 rebuilding teams drawn for each league, the middle taking the rest; `tierGap` 0.33 to 0.31 holds the league's spread (D-40).
- Effect: seed 1, 100 replays and 100 weekly-loop seasons without chains: 62 of 62 measured targets pass. The weekly loop's win sd is 3.02 (was 3.06), best and worst records 14.5 and 2.6, 15-win teams 6.0 a decade and 2-win teams 5.4, perfect or winless 4 per 100 (replays 6); replay blowouts 25.5% (was 25.8%), favorites 66.5%.

## C-23: Experience counts accrued seasons, and M10's final runs (M10 close, 2026-09-26)
- Change: `aging.meanExperience` and `aging.rookiesPerTeam` count accrued seasons (D-37), as the NFL's roster reports count experience and first-year players. No tuning changes.
- Effect: full runs with three 10-season chained leagues each: seed 1 passes 76 of 76 targets and seed 2 passes 76 of 76. Seed 1 and seed 2: experience 4.76 and 4.77 (the band ends at 4.8, and the NFL's three reports average 4.40), first-year players 9.0 and 9.1 a team, average age 26.36 and 26.37, drift −0.1 and −0.3 with the largest group's 1.2 and 1.9, peak ages within 0.4 of each other by group; weekly-loop win sd 3.02 and 3.00. Perfect or winless teams come to 4 and 3 per 100 weekly-loop seasons against the replays' 6 and 7, the low end of the band; M12 judges them on chained seasons (D-40). Retirements average 30.6 years and 6.9 credited seasons. A full run takes about 20 minutes on 4 workers.

## C-24: Draft hit rates over 20-season chains, and M11's final run (M11 close, 2026-09-26)
- Change: calibration follows the chained leagues' own drafts (D-50): each round's multi-year starters against The Hog Sty's study of the 2000 to 2019 drafts, and quarterbacks per first round against Pro Football Reference's 2015 to 2024 first rounds; full runs chain 3 leagues through 20 seasons. Draft value counts the position (D-47, weight 10), and young players' ceilings drift at their first 4 camps (sd 3, D-41). No other tuning changes.
- Effect: seed 1 passes 82 targets and warns on 2, with no failures. Over 12 drafts in each league (about 1,150 picks a round) the rounds' starter rates are 72.8%, 45.8%, 33.7%, 23.1%, 17.6%, 11.4%, and 7.6% (the NFL's 70.6, 49.0, 28.8, 20.1, 14.9, 8.7, 5.9); the same chains without the drift gave 73.1, 46.2, 32.5, 23.1, 17.9, 11.7, 7.7. The warnings: 2.4 quarterbacks per first round (the NFL's 3.5; M14's AI brain and M15's trade-ups for quarterbacks) and average experience 4.90 (the band ends at 4.8), which rose from 4.76 as the chains doubled in length (M12's free agency and re-signing by value). The league's drift is −0.2 and the largest group's 1.2 over 20 seasons; the weekly loop's win sd is 2.99, with 6 perfect or winless teams per 100 seasons. A full run takes about 36 minutes on 4 workers.

## C-25: M12's market, and the final runs with season records on chained seasons (M12 close, 2026-09-26)
- Change: new deals are priced against the market when signed, read before camp, with re-signings and extensions counted, and free agents' first-week prices over their later ones get a band from spec 11.8's softening market. AI teams open their free agency offers at a player's ask on their usual deal plus up to 10% for the most wanted, with D-60's room premium raising only the most they pay (D-65); a player's demand runs 1.2 to 1.4 of his market value (was 1.1 to 1.3); the market curve's middle is 79.75 (quarterbacks 76.75; were 80 and 77); a quarterback's top is Dak Prescott's record against the 2024 cap, 23.5% (was 20%). Season records are judged on chained seasons 3 to 20 (D-40).
- Effect: seed 1 with three 20-season chained leagues passes 94 targets, warns on 2, and fails 4. The economy: new deals at 1.03 times their market value when signed (re-signings about 1.0, free agents who changed teams 1.15, first-week deals 1.09 times the later weeks'), median week 1 cap space 6.1% of the cap and the most 13.4%, dead money 15.4%, cash 104.3%, the richest quarterback deal 20.2%, and free agents' richest 12.0%. The warnings: the richest other deal at 11.2% (the band starts at 12%) and 24.6 net-loss compensatory picks a draft (from 28). Experience passes at 4.61 (was 4.90) and first rounds take 3.1 quarterbacks (was 2.4). The failures are the season records, judged on chained seasons for the first time: the sd of team wins 2.49 (band 2.8 to 3.5), 0.9 teams a decade with 15 or more wins and 1.3 with 2 or fewer, and no perfect or winless team in 54 seasons, while replays give 3.11 and the weekly loop's first seasons 3.08. Chained leagues drift toward parity (docs/KNOWN-ISSUES.md). A full run takes about 39 minutes on 4 workers.
