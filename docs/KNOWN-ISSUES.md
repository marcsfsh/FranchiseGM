...
- Spec 11.6 lists incentives among an offer's terms; an offer carries a per-game roster bonus and one performance incentive a season, each counted at the odds the player gives himself of earning it (D-54, D-64).
## Deferred
...
- Chained leagues drift toward parity: over seasons 3 to 20 the sd of team wins runs about 2.5 against the NFL's 2.8 to 3.5, with no perfect or winless team and about 2 two-win teams a decade (C-25), while replays and a league's first seasons match the NFL. Every team runs on the same AI, and nothing yet keeps a franchise good or bad for long; M13's coaching quality and M14's team modes (contending and rebuilding) should give talent somewhere to stay.
- The richest deal apart from quarterbacks runs about 11.4% of the cap against the NFL's 12% to 18% (a calibration warning, C-25): the other positions' market tops read their records against the 2026 cap, while the quarterback's reads Dak Prescott's against the cap he signed under (D-65). Compensatory picks for net losses run about 25 a draft against the NFL's 28 to 32 (a warning).
...
- Calibration replays use each head coach's auto depth chart and rotation and an AI game plan per game, and hurt players sit out while their backups dress, but nobody goes on injured reserve or gets signed, so every game stays independent (D-17, D-20). Season records are judged on chained seasons 3 to 20 instead, which play through the weekly advance and every offseason (D-24, D-40).
...
- The practice squad's international pathway exemption isn't modeled (no player carries the designation), so squads hold 16. The PUP and NFI lists have their rules, but nothing places players on them yet: players hurt at camp stay on the active roster. Suspensions come only in regular-season weeks (D-59).
- AI teams bid in free agency knowing the least a player would take, while the user's front office sees only a range around it (D-63, D-65); M14's AI brain can estimate prices the way the user's front office does.
